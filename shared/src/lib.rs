...
mod edhoc_parser {
    ...
    pub fn decode_plaintext_2(
        ...
        mac_2[..].copy_from_slice(decoder.bytes_sized(MAC_LENGTH_2)?);        //println!("id_cred_r from plaintext_2:{:?}", id_cred_r);
        ...
        if plaintext_2.len > decoder.position() {
            ...
            if let Ok(ead_2) = ead_res {
                Ok((c_r, None, mac_2, ead_2))
            } else {
                Err(ead_res.unwrap_err())
            }
        } else if decoder.finished() {
            Ok((c_r, None, mac_2, None))
        } else {
            Err(EDHOCError::ParsingError)
        }
    }
