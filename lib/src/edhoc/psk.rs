...
use super::{
    ...
    BufferCiphertext3, BufferMessage3, BufferPlaintext2, BytesHashLen, ConnId, CredentialTransfer,
    DecodedMessage2, EDHOCError, EadItems, IdCred, ParsedMessage2Details, ParsedMessage3,
    PreparedMessage2, PreparedMessage3, ProcessedM2, ProcessedM2MethodSpecifics, ProcessingM2,
    ProcessingM2MethodSpecifics, ProcessingM3, ProcessingM3MethodSpecifics, PskCredential,
    Th4Input, VerifiedMessage2, VerifiedMessage3, WaitM3, WaitM3MethodSpecifics,
};
    ...
    F: Fn(&IdCred) -> Result<PskCredential, EDHOCError>,
{
    ...
    let prk_4e3m = compute_prk_4e3m_psk(crypto, &salt_4e3m, cred_i.psk());
...
pub(crate) fn i_verify_message_2_psk(
    ...
    let prk_4e3m = compute_prk_4e3m_psk(crypto, &salt_4e3m, &valid_cred_r.psk());
...
pub(crate) fn i_prepare_message_3_psk(
    state: &ProcessedM2,
    crypto: &mut impl CryptoTrait,
    cred_i: PskCredential,
    cred_transfer: CredentialTransfer,
    ead_3: &EadItems,
) -> Result<PreparedMessage3, EDHOCError> {
    let id_cred_psk = match cred_transfer {
        CredentialTransfer::ByValue => return Err(EDHOCError::UnexpectedCredential),
