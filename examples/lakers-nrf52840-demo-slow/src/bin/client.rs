...
static mut RX_BUFFER_2: [u8; 36] = [0; 36];
static mut RX_BUFFER_4: [u8; 9] = [0; 9];
const MAX_MSG_SIZE: usize = 38; 

pub const CRED_PSK: &[u8] = &hex!("A202686D79646F74626F7408A101A30104024110205050930FF462A77A3540CF546325DEA214");
...
fn main() -> ! {
    ...
    unsafe {
        ...
        match uart.write(&TX_BUFFER[..msg_1_buf[0..msg_1_len + 1].len()]) {
            Ok(_) => info!("Message_1 (of length {:?}) sent successfully: {}", message_1.as_slice().len() +1,  &TX_BUFFER[..msg_1_buf[0..msg_1_len + 1].len()]),
        ...
        if received_bytes > 0 {
            ...
            if RX_BUFFER_2[0] == 0xF5 {
                ...
                let message_2 = EdhocMessageBuffer::new_from_slice(&RX_BUFFER_2[1..received_bytes]).unwrap();
                ...
                //  Send message_1 over UART
                TX_BUFFER[..message_3.as_slice().len() + 1].copy_from_slice(&msg_3_buf[..msg_3_len + 1]);
                match uart.write(&TX_BUFFER[..message_3.as_slice().len() + 1]) {
                    Ok(_) => info!("Message_3 (of length {:?}) sent successfully: {}", message_3.as_slice().len() + 1 , &TX_BUFFER[..message_3.as_slice().len() + 1]),
                    Err(_) => info!("Failed to send message_3"),
                }
                
                // Wait to receive message_4
                info!("Waiting for message_4");
                RX_BUFFER_4.fill(0);
                let mut received_bytes_4 = 0;
                while received_bytes_4 < RX_BUFFER_4.len() {
                    match uart.read(&mut RX_BUFFER_4[received_bytes_4..received_bytes_4+1]) {
                        Ok(_) => {
                            received_bytes_4 += 1;
                        }
                        Err(_) => break,
                ...
                if received_bytes_4 > 0 {
                    info!("Received {} bytes for message_4", received_bytes_4);
                    info!("Received message_4: {:?}", &RX_BUFFER_4[..received_bytes_4]);
                    let message_4 = EdhocMessageBuffer::new_from_slice(&RX_BUFFER_4[..received_bytes_4]).unwrap();
                    
                    // Process message 4 here if needed
                    ...
                    info!("Handshake completed after receiving message_4");
                    ...
                    info!("Message_4 not received. Handshake incomplete.");
                }

            } else {
                info!("Error: First byte of message_2 is not CBOR true (0xF5), invalid message_2!");
            }
        } else {
