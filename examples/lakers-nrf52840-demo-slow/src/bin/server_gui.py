...
class EdhocServerGUI(QMainWindow):
    ...
    def create_progress_display(self, parent_layout):
        group = QGroupBox("Handshake Progress")
        layout = QVBoxLayout()
        ...
        self.steps = ["message_1", "message_2", "message_3", "message_4", "handshake_complete"]

        for step in self.steps:
            indicator = StatusIndicator(step)
            self.status_layout.addWidget(indicator)
            self.status_indicators[step] = indicator
    ...
    def start_server(self):
        ...
        try:
            ...
            logger.info(f"Server started on {port} at {baud_rate} baud")
    ...
    def update_gui(self, action, data):
        if action == "progress":
            step, completed = data
            if step in self.status_indicators:
                self.status_indicators[step].setStatus(completed)
                ...
                if step == "message_1":
                    ...
                    self.status_indicators[step].setTime(cumulative_time)
                    self.step_times[step] = cumulative_time
                ...
                completed_steps = sum(1 for i in self.status_indicators.values()
                                      if i.indicator.palette().color(QPalette.Window) == QColor(0, 128, 0))
                total_steps = len(self.status_indicators)
                ...
                #     # Optionally quit the app after showing final state
    ...
    def reset_progress(self):
        ...
        self.last_step_time = None
        self.step_times = {}
        self.timer_label.setText("00:00:00.0000")
